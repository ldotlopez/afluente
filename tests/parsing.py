...
class ParsingTest(AssertsMixin, unittest.TestCase):
    def test_episode_parse(self):
        ...
        self.assertEntity(e, kit.Episode, dict(series='lost', season=1, number=1))

    def test_movie_parse(self):
        s, e, t = self.parse('Dark.City.1999.mkv')
        self.assertEntity(e, kit.Movie, dict(title='dark city', modifier='1999'))

    def test_language_found(self):
        # Language code embeded in name
        s, e, t = self.parse('Dark.S01E05.SWE.DUBBED.1080p.WEBRip.x264-SERIOUSLY[rartv]')
    ...
    def test_parse_und_language(self):
        ...
        s, e, t = self.parse('Dark.S01E05.DUBBED.1080p.WEBRip.x264-SERIOUSLY[rartv]')
    ...
    def test_parse_movie_as_episode(self):
        ...
        with self.assertRaises(TypeError):
            s, e, m = self.parse('Al-Jazeera.Canadas.Dark.Secret.720p.HDTV.x264.AAC.mkv[eztv].mkv[eztv]', type='episode')
    ...
    def test_parse_similar_names_match_entities(self):
        ...
        _, e1, _ = self.parse('Dark.S01E05.DUBBED.1080p.WEBRip.x264-SERIOUSLY[rartv]')
        _, e2, _ = self.parse('Dark.S01E05.HDTV.KILLERS.PROPER[rartv]')
